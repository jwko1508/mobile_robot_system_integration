...
import threading
import time
from snowboydecoder import HotwordDetector as ht
...
class snowboy_multy(object):
    def __init__(self, modelName = ['hello.pmdl','finish.pmdl','stop.pmdl','start.pmdl'],
                    callbackfunc = snowboydecoder.play_audio_file):
        model = [curpath + modelpath + modelName[0], 
    ...
    # def number(self):
    #     self.num = self.detector.start().ans
    ...
    def _startSnowDetector(self):
        self.ans_num = self.detector.start(detected_callback= self.callbackfun,
                       interrupt_check= self.interrupted_callback,
                       sleep_time= 0.03)
        self.detector.terminate()
        # print('terminate')
