...
#### 오류 고침
- 오류 고치고 서비스 클라이언트 만듬.
- 이 서비스 파일 만드는데 3일 머리 애워쌈.
- 엄청 간단한 해결법을 매우 오래 붙잡아둠.
#### 오류 또 고침
- 갑자기 또 오류 엄청 생겨서 또 고침.
- 이번에도 오류나면 화날거 같음.
- 서비스 노드 만들어서 실험해봄.
- srv파일 전용 패키지 만듬.
...
## author : jwko1508
#### 음성엔진 snowboy로 교체
- snowboy로 단어 인식으로 교체함
#### 음성제너레이터 gtts 오류
- 이것도 인터넷연결이 되어있지 않다면 실행이 되지 않기 때문에 음성제너레이터로 만든 mp3 파일을 저장한뒤 실행하는 것으로 작성함
#### 서비스까지 일단 완성
#### 집의 마이크 상태 최저
- 음성 모델에 사용한 음성녹음의 상태가 매우 좋지않아 음성인식률이 좋지않음
- 바람소리만 들려도 피터팬이 인식됨
- 나중에 마이크를 바꿔서 인식할 예정
...
## author : jwko1508
#### 녹음 다시해서 정확도 개선
- 그외에 민감도 조절해서 실험해봄 구글만큼은 아니지만 잘 인식함
